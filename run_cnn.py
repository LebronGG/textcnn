...
def train():
    ...
    x_train, y_train, train_contents, train_labels = process_file(train_dir, word_to_id, cat_to_id, config.seq_length)
    x_val, y_val, val_contents, val_labels = process_file(val_dir, word_to_id, cat_to_id, config.seq_length)
    time_dif = get_time_dif(start_time)
    print("Time usage:", time_dif)

    # 创建session
    session = tf.Session()
    if not load_checkpoint(save_dir, session):
        ...
        for x_batch, y_batch in batch_train:
            ...
            if total_batch % config.print_per_batch == 0:
                ...
                if acc_val > best_acc_val:
                    ...
                    save_path = os.path.join(save_dir, 'model-{:.3}'.format(acc_val))
                ...
                msg = 'Iter: {0:}, Train Loss:{1:.2}, Train Acc:{2:.2%},' \
                      + ' Val Loss:{3:.2}, Val Acc:{4:.2%}, Time: {5} {6}'
...
def test():
    ...
    start_time = time.time()
    x_test, y_test, test_contents, test_labels = process_file(test_dir, word_to_id, cat_to_id, config.seq_length)
    ...
    msg = 'Test Loss: {0:>6.3}, Test Acc: {1:>7.3%}'
